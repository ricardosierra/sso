...
use App\Exceptions\SSOServerException;
...
class LaravelSSOServer
{
    ...
    {
        if (!Auth::attempt([config('sso.userUserNameField') => $username, 'password' => $password])) {
            return false;
        }
    ...
    {
        try {
            $broker = config('sso.brokersModel')::where('name', $brokerId)->firstOrFail();
    ...
    {
        try {
            $user = config('sso.usersModel')::where(config('sso.userUserNameField'), $username)->firstOrFail();
        } catch (ModelNotFoundException $e) {
            return null;
        }
